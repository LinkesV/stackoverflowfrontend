...
function Adduser() {
  ...
  const submitHandler = async ()=>{
    try{
      if(fname.length == 0 || lname.length == 0 || aboutme.length == 0){
      ...
      if(result){
        await fetch("http://localhost:4000/usersignup", {
                method: "POST", 
                headers: {
  ...
  return (
    <div className='loginbody'>
        ...
        <input type="text" placeholder="Detials about yourself" name="about" required onChange={(e)=>{setAboutme(e.target.value)}}/><br></br>
